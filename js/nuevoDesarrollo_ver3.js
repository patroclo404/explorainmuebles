...
$(document).ready(function(){
	...
	if (typeof post_tipo !== 'undefined') {
		$("#desarrollo_inmobiliaria li.lista li[data-value='"+post_inmobiliaria+"']").click();
		$("#desarrollo_inmobiliaria li.lista").hide();
		...
		if (!vacio((parseInt($("#desarrollo_tipo p").attr("data-value")) == -1 ? "" : $("#desarrollo_tipo p").attr("data-value")), "Tipo")) {
			if (!vacio($("#desarrollo_descripcion").val(), $("#desarrollo_descripcion").attr("placeholder"))) {
				if ($("#_desarrolloLatitud").val() != "") {
					if (!vacio((parseInt($("#desarrollo_inmobiliaria p").attr("data-value")) == -1 ? "" : $("#desarrollo_inmobiliaria p").attr("data-value")), "Inmobiliaria")) {
...
function save() {
	...
	$("#_desarrolloInmobiliaria").val($("#desarrollo_inmobiliaria p").attr("data-value"));
	...
	$("#subirDesarrollo").ajaxSubmit({
		...
		success: function(respuesta_json){
			if (parseInt($("#idDesarrollo").val()) == -1)
				gotoURL("desarrollo.php?id="+respuesta_json.id+"&create=1");
			else
				gotoURL("desarrollo.php?id="+$("#idDesarrollo").val());
		}
	});
}
/**/