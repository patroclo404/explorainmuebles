...
function misAnuncios_popupRenovar(idInmueble, tipo) {
	...
	if (parseFloat(_precioPublicar) > 0) {
		$("#_enviar").attr("onclick", "gotoURL('pagoInmueble.php?idInmueble="+idInmueble+"');");
		$("#_tipo").text(tipo);
	}
	else {
		$("#_enviar").attr("onclick", "misAnuncios_pagoInmediato("+idInmueble+");");
		$("#_tipo").text(tipo);
	}
}
...
/*
	Realiza el pago inmediato, esto es ya que el precio del anuncio es de 0
	
		* idInmueble:	Integer, es el id del inmueble a desactivar 
*/
function misAnuncios_pagoInmediato(idInmueble) {
	$.ajax({
