			...
			click: function() {
				...
				if (parseInt($(this).attr("data-value")) != -1) {
					...
					if (parseInt($(this).attr("data-value")) == 3) {
						...
						$("#crearAnuncio_transaccion").css("visibility", "hidden");
					}
					else {
						$("#crearAnuncio_transaccion").css("visibility", "visible");
...
function nuevoAnuncio_tempImagenCargada(nombreArchivo) {
	...
	if (imagenesTemporales.find(".bloqueImagen").length < 7) {
												...
												if (!vacio($("#crearAnuncio_descripcion").val(), $("#crearAnuncio_descripcion").attr("placeholder"))) {
													if (!vacio((parseInt($("#crearAnuncio_estadoConservacion p").attr("data-value")) == -1 ? "" : $("#crearAnuncio_estadoConservacion p").attr("data-value")), "Estado de Conservación")) {
														...
														if (!isVacio($("#crearAnuncio_metrosFrente").val())) {
															if (!entero($("#crearAnuncio_metrosFrente").val(), $("#crearAnuncio_metrosFrente").attr("placeholder"))) {
																return false;
															}
														}
														
														if (!isVacio($("#crearAnuncio_metrosFondo").val())) {
															if (!entero($("#crearAnuncio_metrosFondo").val(), $("#crearAnuncio_metrosFondo").attr("placeholder"))) {
																return false;
															}
														}
