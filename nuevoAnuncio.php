<?php
	...
	$consulta = "SELECT USU_TELEFONO1, USU_TELEFONO2, USU_CALLE_NUMERO, USU_ESTADO, USU_CIUDAD, USU_COLONIA FROM USUARIO WHERE USU_ID = ?;";
	...
	if (isset($_POST["edit"])) {
		...
		$pdo = $conexion->prepare($consulta);
		$pdo->execute(array(":usuario" => $usuario["id"], ":id" => $_POST["id"]));
		$res = $pdo->fetchAll(PDO::FETCH_ASSOC);
		$row = $res[0];
		$inmueble = array(
			"id"						=>	$row["IMU_ID"],
	...
	CabeceraHTML("nuevoAnuncio_ver11.css,nuevoAnuncio_ver22.js", $variables);
	CuerpoHTML();
?>
                            ...
                            <td>
                            	<div class="contenedorCampos">
                                    <ul id="crearAnuncio_wcs" class="template_campos">
                                    	Baños<a class="obligatorio">*</a><span></span>
                                        <li class="lista">
                                        	<ul>
												<?php
													for ($x = 1; $x <= 10; $x++) {
                                ...
                                <div class="contenedorCampos" style="display:none;">
                                    <input type="text" id="_crearAnuncioDesarrollo" name="desarrollo" value="<?php echo $edit == 1 ? $inmueble["desarrollo"] : $desarrollo; ?>" style="display:none;" />
                                </div>
                            </td>
                            <td>
                            	<div class="contenedorCampos">
                                    <ul id="crearAnuncio_recamaras" class="template_campos">
                                    	Recamaras<a class="obligatorio">*</a><span></span>
                                        <li class="lista">
                                        	<ul>
												<?php
													for ($x = 1; $x <= 10; $x++) {
