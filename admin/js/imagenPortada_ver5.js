...
function mostrarCamposExistentes() {
	...
	for (var pos = 0; pos < positions.length; pos++) {
		...
		divImagen.innerHTML = 
			"<table>"+
				"<tr>"+
					"<td width='"+$("#template_nombreCampos").find("td").eq(0).width()+"'><a href='javascript:abrirModificarCampos("+pos+");'>Modificar</a></td>"+
					"<td style='text-align:center;'><a href='"+urlArchivos+positions[pos][1]+"' target='_blank'>Imágen: "+positions[pos][1]+"</a></td>"+
					"<td width='"+$("#template_nombreCampos").find("td").eq(2).width()+"' style='text-align:center;'>"+(parseInt(positions[pos][2]) == 0 ? "-" : positions[pos][2])+"</td>"+
					"<td width='15'>"+
...
function abrirModificarCampos(posit){
	if(!bool_borrar){
		...
		if(posit >= 0){
			...
			$("#textoPrincipal").val(positions[pos_comp][4]);
			$("#textoSecundario").val(positions[pos_comp][5]);
		}
		else{
			pos_comp = -1;
