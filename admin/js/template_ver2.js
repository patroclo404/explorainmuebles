...
/*
	De la url actual, se obtienen todos los parametros y se guarda como un arreglo de parametros del tipo: {key: value},
	
		* key:	[String], puede ser null, en caso de que no se obtenga ningun key, devuelve el arreglo completo de tipo: key/value
				Si se recibe un key, devuelve undefined si no existe el key; en caso de que si exista, su valor correspondiente
*/
function template_getURL_keyValue(key) {
	_getURL = document.URL+"";
	_soloParametros = _getURL.substr(_getURL.indexOf("?")+1, _getURL.length);
	arrayParametros = {};
	
	if (_soloParametros.length > 0) {
		_params = _soloParametros.split("&");
		
		for (var x = 0; x < _params.length; x++) {
			partes = _params[x].split("=");
			arrayParametros[partes[0]] = partes[1];
		}
	}
	
	
	if (key != null)
		return arrayParametros[key];
