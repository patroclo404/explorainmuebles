...
function abrirModificarCampos(posit){
	if(!bool_borrar){
		...
		if(posit >= 0){
			...
			$("#telefono1").val(positions[posit][5]);
			$("#telefono2").val(positions[posit][6]);
			$("#calleNumero").val(positions[posit][7]);
			
			usuario_onChange_estado_ciudad_colonia(positions[posit][8], positions[posit][9], positions[posit][10]);
		}
		else{
			pos_comp = -1;
