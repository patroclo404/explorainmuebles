		...
		divImagen.innerHTML = 
			"<table>"+
				"<tr>"+
					...
					"<td width='"+$("#template_nombreCampos").find("td").eq(6).width()+"' style='text-align:center;'>"+positions[pos].creditos+"</td>"+
					"<td width='"+$("#template_nombreCampos").find("td").eq(7).width()+"' style='text-align:center;'><a href='javascript:inmobiliaria_abrirModificarPagos("+pos+");'>Pagos</a></td>"+
					"<td width='15'>"+
...
function inmobiliaria_cerrarPopUp(){
	...
	$("#inmobiliaria_abrirModificarPagos").hide();
}
...
/*
	Muestra un popup para generar un nuevo pago a la inmobiliaria
	
		* posit:	Integer, es el id de la inmobiliaria
*/
function inmobiliaria_abrirModificarPagos(posit) {
	$("#mascaraPrincipal").show();
	
	var objDiv = $("#inmobiliaria_abrirModificarPagos");
	...
	$("#inmobiliariaPago").text(positions[pos_comp].nombreEmpresa);
	...
	_fecha = new Date();
	$("#validez2").pickadate().pickadate("picker").set("select", [_fecha.getFullYear(), _fecha.getMonth(), _fecha.getDate()]);
}
...
/*
	Valida los campos para crear un nuevo pago a la inmobiliaria
*/
function validarCampos3() {
	if (!vacio($("#creditos2").val(), $("#creditos2").attr("placeholder"))) {
		if (entero($("#creditos2").val(), $("#creditos2").attr("placeholder"))) {
			if (!vacio($("#total").val(), $("#total").attr("placeholder"))) {
				if (!vacio($("#validez2").val(), $("#validez2").attr("placeholder"))) {
...
/*
	Guarda el pago a la inmobiliaria
*/
function savePagos() {
	$("#btnGuardar3").hide();
	$("#mensajeTemporal3").show();
	$("#inmobiliaria_abrirModificarPagos").css({cursor:"wait"});
	...
	$.ajax({
		...
		data: {
			id: -1,
			creditos: $("#creditos2").val(),
			total: $("#total").val(),
			validez: $("#validez2").val(),
			tipo: $("#tipo").val(),
			inmobiliaria: positions[pos_comp].id,
		...
		if (respuesta_json.isExito == 1) {
			...
			$("#inmobiliaria_abrirModificarPagos").css({cursor:"default"});
