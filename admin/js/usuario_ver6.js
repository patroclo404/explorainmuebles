...
function abrirModificarCampos(posit){
	if(!bool_borrar){
		...
		if(posit >= 0){
			...
			if (positions[posit][17] != "") {
				$("#imagenLogoTipo").prop("href", urlArchivos+positions[posit][17]);
				$("#imagenLogoTipo").show();
			}
			if (positions[posit][18] != "") {
				$("#imagenActual").prop("href", urlArchivos+positions[posit][18]);
				$("#imagenActual").show();
			}
		}
		else{
