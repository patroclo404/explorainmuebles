		...
		divImagen.innerHTML = 
			"<table>"+
				"<tr>"+
					...
					"<td width='"+$("#main table.main_table tr").eq(2).find("td").eq(4).width()+"' style='text-align:center;'><a href='javascript:inmueble_abrirModificarImagenes("+pos+");'>Imágenes</a></td>"+
					"<td width='"+$("#main table.main_table tr").eq(2).find("td").eq(5).width()+"' style='text-align:center;'><a href='javascript:inmueble_abrirModificarVideos("+pos+");'>Videos</a></td>"+
					"<td width='15'>"+
...
function inmueble_inicializarBotones() {
	...
	$("input[name='pertenece']").on({
		change: function() {
			if ($("input[name='pertenece']:checked").val() == "V") {
				$("#vendedor").val(-1);
				$("#vendedor").show();
				$("#inmobiliaria").hide();
			}
...
function abrirModificarCampos(posit){
	if(!bool_borrar){
		...
		if(posit >= 0){
			...
			inmueble_onChange_pertenece(positions[pos_comp].vendedor, positions[pos_comp].inmobiliaria);
			...
			$("input[name='pertenece'][value='V']").prop("checked", true);
...
/*
	Actualiza los campos de: vendedor e inmobiliaria
	
		* vendedor:		[integer], es el id del vendedor
		* inmobiliaria:	[integer], es el id de la inmobiliaria
*/
function inmueble_onChange_pertenece(vendedor, inmobiliaria) {
	$("input[name='pertenece'][value='"+(vendedor != "" ? "V" : "I")+"']").prop("checked", true);
	$("input[name='pertenece']").change();
	$("#vendedor").val(vendedor);
	$("#inmobiliaria").val(inmobiliaria);
}
...
function validarCampos () {
	var arrayCamposEvaluar;
	var arrayCamposObligatorios;
	
	if ($("input[name='pertenece']:checked").val() == "V") {
		arrayCamposEvaluar = new Array("titulo", "vendedor", "categoria", "tipo", "precio", "calleNumero", "estado", "ciudad", "colonia", "latitud", "longitud", "codigo", "dimensionTotal", "dimensionConstruida", "dimensionUtil", "estadoConservacion", "amueblado", "cuotaMantenimiento", "elevador", "estacionamientoVisitas", "numeroOficinas");
		arrayCamposObligatorios = new Array("titulo", "vendedor", "categoria", "tipo", "precio", "calleNumero", "estado", "ciudad", "colonia", "latitud", "longitud", "codigo", "estadoConservacion");
	}
	else {
		arrayCamposEvaluar = new Array("titulo", "inmobiliaria", "categoria", "tipo", "precio", "calleNumero", "estado", "ciudad", "colonia", "latitud", "longitud", "codigo", "dimensionTotal", "dimensionConstruida", "dimensionUtil", "estadoConservacion", "amueblado", "cuotaMantenimiento", "elevador", "estacionamientoVisitas", "numeroOficinas");
		arrayCamposObligatorios = new Array("titulo", "inmobiliaria", "categoria", "tipo", "precio", "calleNumero", "estado", "ciudad", "colonia", "latitud", "longitud", "codigo", "estadoConservacion");
	}
...
function save(){
	...
	var arrayTodosCampos = new Array("titulo", "vendedor", "inmobiliaria", "categoria", "tipo", "precio", "calleNumero", "estado", "ciudad", "colonia", "latitud", "longitud", "descripcion", "antiguedad", "codigo", "dimensionTotal", "dimensionConstruida", "dimensionUtil", "estadoConservacion", "amueblado", "cocinaEquipada", "estudio", "cuartoServicio", "cuartoTV", "bodega", "terraza", "jardin", "areaJuegosInfantiles", "comedor", "serviciosBasicos", "gas", "lineaTelefonica", "internetDisponible", "aireAcondicionado", "calefaccion", "cuotaMantenimiento", "casetaVigilancia", "elevador", "seguridad", "alberca", "casaClub", "canchaTenis", "visitaMar", "jacuzzi", "estacionamientoVisitas", "permiteMascotas", "gimnasio", "centrosComercialesCercanos", "escuelasCercanas", "fumadoresPermitidos", "numeroOficinas");
