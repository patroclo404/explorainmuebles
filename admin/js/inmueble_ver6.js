...
function inmueble_inicializarBotones() {
	...
	$("#chkCuotaMantenimiento,#chkElevador,#chkEstacionamientoVisitas,#chkNumeroOficinas,#chkWcs,#chkRecamara").on({
		change: function() {
	...
	$("#usuario").on({
		change: function() {
			if ($(this).val() != -1) {
				if ($("#usuario option:selected").attr("data-inmobiliaria") == 1) {
					$("#celdaCodigo").show();
				}
				else {
					$("#celdaCodigo").hide();
...
function abrirModificarCampos(posit){
	if(!bool_borrar){
		...
		if(posit >= 0){
			...
			var arrayTodosCampos = new Array("titulo", "usuario", "categoria", "tipo", "precio", "calleNumero", "estado", "ciudad", "colonia", "latitud", "longitud", "descripcion", "antiguedad", "codigo", "dimensionTotal", "dimensionConstruida", "estadoConservacion", "amueblado", "cocinaEquipada", "estudio", "cuartoServicio", "cuartoTV", "bodega", "terraza", "jardin", "areaJuegosInfantiles", "comedor", "serviciosBasicos", "gas", "lineaTelefonica", "internetDisponible", "aireAcondicionado", "calefaccion", "cuotaMantenimiento", "casetaVigilancia", "elevador", "seguridad", "alberca", "casaClub", "canchaTenis", "vistaMar", "jacuzzi", "estacionamientoVisitas", "permiteMascotas", "gimnasio", "centrosComercialesCercanos", "escuelasCercanas", "fumadoresPermitidos", "numeroOficinas", "wcs", "recamaras");
			...
			$("#tipo").change();
			inmueble_onChange_chkCamposNumericos(positions[pos_comp].cuotaMantenimiento, positions[pos_comp].elevador, positions[pos_comp].estacionamientoVisitas, positions[pos_comp].numeroOficinas, positions[pos_comp].wcs, positions[pos_comp].recamaras);
		}
		else{
			pos_comp = -1;
			...
			$("#estado").change();
			$("#tipo").change();
			$("#chkCuotaMantenimiento,#chkElevador,#chkEstacionamientoVisitas,#chkNumeroOficinas,#chkWcs,#chkRecamara").change();
		}
...
/*
	Muestra u oculta los campos de tipo numerico dependiendo del valor recibido
		...
		* wcs:						[Integer], es el numero de baños
		* recamara:					[Integer], es el numero de recamaras
*/
function inmueble_onChange_chkCamposNumericos(cuotaMantenimiento, elevador, estacionamientoVisitas, numeroOficinas, wcs, recamara) {
	...
	$("#chkWcs").prop("checked", (wcs != "" ? true : false));
	$("#chkRecamara").prop("checked", (recamara != "" ? true : false));
	$("#chkCuotaMantenimiento,#chkElevador,#chkEstacionamientoVisitas,#chkNumeroOficinas,#chkWcs,#chkRecamara").change();
}
...
function validarCampos () {
	var arrayCamposEvaluar = new Array("titulo", "usuario", "categoria", "tipo", "precio", "calleNumero", "estado", "ciudad", "colonia", "latitud", "longitud", "codigo", "dimensionTotal", "dimensionConstruida", "estadoConservacion", "amueblado", "cuotaMantenimiento", "elevador", "estacionamientoVisitas", "numeroOficinas", "wcs", "recamaras");
	var arrayCamposObligatorios = new Array("titulo", "usuario", "categoria", "tipo", "precio", "calleNumero", "estado", "ciudad", "colonia", "latitud", "longitud", "estadoConservacion");
	
	if ($("#celdaCodigo").css("display") != "none") {
		arrayCamposObligatorios.push("codigo");
	}
	
	var arrayFlotantes = new Array("precio", "dimensionTotal", "dimensionConstruida", "cuotaMantenimiento");
	var arrayEnteros = new Array("elevador", "estacionamientoVisitas", "numeroOficinas", "wcs", "recamaras");
