...
function mostrarCamposExistentes() {
	...
	for (var pos = 0; pos < positions.length; pos++) {
		...
		var textInmobiliaria = (positions[pos][19]).length > maxCadena ? ((positions[pos][19]).substr(0, (maxCadena - 3))+"...") : positions[pos][19];
		maxCadena = 35;
		contPendientes += positions[pos][16] != 1 ? 1 : 0;
		...
		divImagen.innerHTML = 
			"<table>"+
				"<tr>"+
					...
					"<td width='"+$("#template_nombreCampos").find("td").eq(3).width()+"' style='text-align:center;'>"+positions[pos][0]+"</td>"+
					"<td width='"+$("#template_nombreCampos").find("td").eq(4).width()+"' style='text-align:center;'>"+(parseInt(positions[pos][18]) > 0 ? ("<a href='javascript:usuario_abrirModificarInmuebles("+pos+");'>"+positions[pos][18]+"</a>") : positions[pos][18])+"</td>"+
					"<td width='"+$("#template_nombreCampos").find("td").eq(5).width()+"' style='text-align:center;'>"+positions[pos][17]+"</td>"+
					"<td width='"+$("#template_nombreCampos").find("td").eq(6).width()+"' style='text-align:center;'>"+(textInmobiliaria != "" ? ("<a href='inmobiliaria.php?idInmobiliaria="+positions[pos][13]+"'>"+textInmobiliaria+"</a>") : "")+"</td>"+
					"<td width='15'>"+
	...
	if (positions.length > 0) {
		$("#_numUsuarios").text(positions.length);
		$("#_numPendientes").text(contPendientes);
	}
}
...
function usuario_inicializarBotones() {
	$("#template_celdaBuscador").find("td").eq(0).append("<p style='padding-top:15px; padding-bottom:15px;'><a style='margin-right:100px;'>Total de usuarios: <span id='_numUsuarios'>0</span></a><a href='usuario.php?pendientes=1'>Pendientes por validar su correo: <span id='_numPendientes'>0</span></a></p>");
